...
function surfaceButtonClass(surfaceId) {
  return [
    'h-10 w-full text-xs font-semibold rounded-md border transition-colors duration-150',
...
<template>
  <div class="w-full max-w-[360px] space-y-2 p-2.5 md:p-3 bg-white rounded-lg border border-gray-200 shadow-sm">
    <div class="flex items-center justify-between">
      <label class="text-sm font-semibold text-gray-800">2. Гадаргуу сонгох</label>
    </div>

    <div class="grid grid-cols-3 gap-1 w-[150px] sm:w-[170px] mx-auto">
      <div></div>
      <button
        type="button"
        :disabled="disabled"
    ...
    <p v-else class="text-xs text-gray-500">Гадаргуу сонгогдоогүй.</p>
  </div>
</template>
