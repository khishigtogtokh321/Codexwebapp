...
const filteredTreatmentTypes = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(
      (t) =>
        t.id.toLowerCase().includes(query) || t.label.toLowerCase().includes(query)
    )
  }
...
<template>
  <div class="space-y-3 p-4 bg-gradient-to-br from-gray-50 to-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-300">
    ...
    <!-- Selected treatment -->
    <p v-if="selectedType" class="text-xs text-purple-600 font-medium flex items-center gap-1">
      <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
      </svg>
      Сонгосон: {{ treatmentTypes.find((t) => t.id === selectedType)?.label }}
    </p>
  </div>
</template>
